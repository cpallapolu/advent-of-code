...
- During the 20th cycle, register **`X`** has the value **`21`**, so the signal strength is 20 \* 21 = **_`420`_**. (The 20th cycle occurs in the middle of the second **`addx -1`**, so the value of register **`X`** is the starting value, `1`, plus all of the other **`addx`** values up to that point: 1 + 15 - 11 + 6 - 3 + 5 - 1 - 8 + 13 + 4 = 21.)
